<div class="p-4 pt-[15px] border-[#e4dfdf]">
    <h4 class="text-center mb-[15px]">{{ __('Recent activities') }}</h4>
    ...
    @if(count($activities))
        <div class="inline-block w-[90%] mt-4 px-2 py-2">
            <a href="
            @if(isset($student)) {{ route('activities', ['student_id' => $user->id]) }}
            @else {{ route('activities', ['librarian_id' => $user->id]) }} @endif
            " class="btn-animation block text-center w-full px-4 py-2 text-sm tracking-wider
