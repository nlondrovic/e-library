...
@section('main')
    ...
    <div class="pl-[50px] pb-[5px] border-b-[1px] border-[#e4dfdf] header-breadcrumbs">
        ...
        <a href="{{ route('sizes.edit', $size) }}">{{ $size->name }}</a> >
        <a href="{{ route('sizes.edit', $size) }}">{{ __('Edit') }}</a>
    </div>

    <div class="scroll height-content section-content">
        <form class="text-gray-700 forma pl-[28px]" method="post" action="{{ route('sizes.update', $size) }}" id="not_filter_form">
            @csrf
            @method('put')
