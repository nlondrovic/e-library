...
@section('main')
    ...
    <div class="pl-[50px] pb-[5px] border-b-[1px] border-[#e4dfdf] header-breadcrumbs">
        <h1> {{__('New publisher')}}</h1>
        <a href="{{ route('dashboard') }}">{{ __('Home') }}</a> >
        <a href="{{ route('policy.index') }}">{{ __('Settings') }}</a> >
        <a href="{{ route('publishers.index') }}">{{ __('Publishers') }}</a> >
        <a href="{{ route('publishers.create') }}">{{ __('New publisher') }}</a>
    </div>

    <div class="scroll height-content section-content pl-[32px]">
        <form class="text-gray-700 forma" method="post" action="{{ route('publishers.store') }}" id="not_filter_form">
            @csrf
            @method('post')
                ...
                <div class="w-[50%] mb-[100px]">
                    <div class="mt-[20px]">
                        ...
                        <input type="text" name="name" required
                               ...
                               placeholder="{{ __('Enter a publisher name') }}"/>
